...
func TestUnmeasuredIsForwarded(t *testing.T) {
	assert.Equal(t, 6, gatedreporter.New(fakelogger.New(), fakeGates{}, fakereporter.Counting{Outside: 6}).Unmeasured(),
...
func TestAnUnreadableUnmeasuredCountIsNotZero(t *testing.T) {
	assert.Equal(t, -1, gatedreporter.New(fakelogger.New(), fakeGates{}, fakereporter.Countless{}).Unmeasured())
}
