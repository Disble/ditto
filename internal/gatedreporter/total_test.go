...
func TestTotalIsForwarded(t *testing.T) {
	assert.Equal(t, 47, gatedreporter.New(fakelogger.New(), fakeGates{}, fakereporter.Counting{Scored: 47}).Total(), "a decorator that drops a capability is refused by nothing")
}
...
func TestAnUnreadableCountIsNotZero(t *testing.T) {
	assert.Equal(t, -1, gatedreporter.New(fakelogger.New(), fakeGates{}, fakereporter.Countless{}).Total())
}
